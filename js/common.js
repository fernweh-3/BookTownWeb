...
function getCaptcha(time) {
    ...
    xhr.open("GET", "/api/captcha?" + time, true);
    xhr.responseType = "blob";
    const imageElement = document.createElement("img");
    imageElement.setAttribute("id","captcha_img")
    xhr.onload = function () {
        if (this.status === 200) {
...
function httpAjax(method, url, data, flag) {
    ...
    xmlHttp.onreadystatechange = () => {  //每当 readyState 属性改变时，就会调用该函数
        if (xmlHttp.readyState === 4 && xmlHttp.status === 200) {
            if (xmlHttp.getResponseHeader("Content-Type") === "image/gif") {
                const base64 = window.btoa(responseText);
            } else {
...
function popup(message, confirmOp, confirmContent = "确定", closeContent = "close") {
    ...
    setVisible(layer, popupEle)
    warning_content.innerText = message;
    close.onclick = function () {
        setInvisible(layer, popupEle)
    }
